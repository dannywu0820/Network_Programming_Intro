...
Incident *head = NULL, *curr = NULL, *prev = NULL;

void Insert(int r, int w, int h){
     struct Incident *temp = (Incident *)malloc(sizeof(Incident));
     temp->round = r;
     temp->who = w;
     temp->how_much = h;
     temp->next = NULL;
     ...
     if(curr == NULL){
          ...
          while(curr != NULL && curr->round <= temp->round){
              prev = curr;
              curr = curr->next;
          }
          if(curr == head){ //add to head
              temp->next = curr;
              head = temp;
              curr = head;
          }
...
int main(int argc, char *argv[]){
    ...
    while(1){
        ...
        if(head == NULL) printf("No Incident\n");
        else{
            struct Incident *now = head, *last = NULL;
            while(now != NULL && now->round <= Round){
                printf("%d %d %d->", now->round, now->who, now->how_much);
                if(now->who == 0){
