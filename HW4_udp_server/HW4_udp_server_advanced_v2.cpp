...
void Insert(int r, int w, long long int h){
    ...
    if(curr == NULL){
        ...
        if(curr == head){ //new node is added to first
            temp->next = curr;
            head = temp;
            curr = head;
        }
...
void Incident_Check(){
    curr = head, prev = NULL;
    if(curr != NULL){
        while(curr != NULL && curr->round <= Round){
            if(curr->which == 0){
                for(int i = 0; i < num_of_accounts; i++) A[i].deposit = 0;
            }
            else{
                for(int i = 0; i < num_of_accounts; i++){
                    if(A[i].name == curr->which){
                        A[i].deposit+=curr->how_much;
                        break;
                    }
                }
            }
        ...
        if(strcmp(s1, "save") == 0){ //save
            ...
            for(int i = 0; i < num_of_accounts; i++){
                if(A[i].port == client_port){
                    ...
                    else {A[i].deposit-=money; invalid = 0;}
